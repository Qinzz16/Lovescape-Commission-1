...
import { getCollectionForEdit, isMonthLocked, listStaff } from "@/lib/queries";
import { EditableAllocations } from "@/components/editable-allocations";
import { Notice, PageHead, Status } from "@/components/ui";

export default async function EditCollection({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  await requireAdmin();
  const [{ id }, q] = await Promise.all([params, searchParams]);
  const item = await getCollectionForEdit(id);
  if (!item) notFound();
  const [locked, people] = await Promise.all([
