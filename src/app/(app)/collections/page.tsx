  ...
  return <>
    ...
    <section className="section">
      ...
      <tbody>{Array.from(grouped.values()).map(({ base, allocations }) => <tr key={base.collection.id}>
        ...
        <td>{allocations[0]?.staffId ? allocations.map((a) => <div key={a.staffId}>{a.staffName}: <Money value={a.allocatedCollectedSen} /> {base.collection.bookitPaymentId ? <>· Bookit commission = <strong><Money value={a.commissionAmountSen} /></strong></> : <>× {(a.commissionRateBps / 100).toFixed(2)}% = <strong><Money value={a.commissionAmountSen} /></strong></>}</div>) : <span className="muted">No staff allocation · no commission</span>}</td>
