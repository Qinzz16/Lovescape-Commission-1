  ...
  return <>
    ...
    <section className="section">
      ...
      <tbody>{Array.from(grouped.values()).map(({ base, allocations }) => <tr key={base.collection.id}>
        ...
        <td>{allocations[0]?.staffId ? allocations.map((a) => <div key={a.staffId ?? "unassigned"}>{a.staffName ?? "Unassigned"}: {a.allocatedCollectedSen == null ? "—" : <Money value={a.allocatedCollectedSen} />} {base.collection.bookitPaymentId ? <>· Bookit commission = <strong>{a.commissionAmountSen == null ? "—" : <Money value={a.commissionAmountSen} />}</strong></> : <>× {a.commissionRateBps == null ? "—" : `${(a.commissionRateBps / 100).toFixed(2)}%`} = <strong>{a.commissionAmountSen == null ? "—" : <Money value={a.commissionAmountSen} />}</strong></>}</div>) : <span className="muted">No staff allocation · no commission</span>}</td>
        <td>{base.collection.source.replace("_", " ")}{base.collection.bookitPaymentId ? <div className="muted">Payment: {base.collection.bookitPaymentId}</div> : null}</td>
