...
export async function createCollectionAction(form: FormData) {
  ...
  const [created]=await getDb().insert(collections).values({orderId:orderId||null,customerName:customerName||null,weddingPickupDate,collectionDate,category,collectedSen,source:s(form,"source")==="MANUAL_ADJUSTMENT"?"MANUAL_ADJUSTMENT":"BOOKIT",notes:s(form,"notes")||null,createdBy:admin.id}).returning();
  const splits=splitAmount(collectedSen,allocations); const allocationRows=splits.map(a=>({collectionId:created.id,staffId:a.staffId,allocationBps:a.allocationBps,allocatedCollectedSen:a.amountSen,commissionRateBps:rate,commissionAmountSen:calculateCommission(a.amountSen,rate)})); await getDb().insert(collectionAllocations).values(allocationRows); await getDb().insert(commissionPortions).values(createCommissionPortionRows(created.id,allocationRows,collectionDate,weddingPickupDate,settings));
...
export async function updateCollectionAction(form: FormData) {
  ...
  const customerName=s(form,"customerName") || null; const weddingPickupDate=s(form,"weddingPickupDate") || null; const settings=await getSettings();
  await getDb().transaction(async tx=>{ await tx.delete(paymentScheduleAllocations).where(eq(paymentScheduleAllocations.collectionId,id)); await tx.update(collections).set({orderId:s(form,"orderId")||null,customerName,weddingPickupDate,collectionDate,category,collectedSen,source:s(form,"source")==="MANUAL_ADJUSTMENT"?"MANUAL_ADJUSTMENT":"BOOKIT",notes:s(form,"notes")||null,updatedAt:new Date()}).where(eq(collections.id,id)); await tx.delete(collectionAllocations).where(eq(collectionAllocations.collectionId,id)); await tx.delete(commissionPortions).where(eq(commissionPortions.collectionId,id)); const splits=splitAmount(collectedSen,allocations); const allocationRows=splits.map(a=>({collectionId:id,staffId:a.staffId,allocationBps:a.allocationBps,allocatedCollectedSen:a.amountSen,commissionRateBps:rate,commissionAmountSen:calculateCommission(a.amountSen,rate)})); await tx.insert(collectionAllocations).values(allocationRows); await tx.insert(commissionPortions).values(createCommissionPortionRows(id,allocationRows,collectionDate,weddingPickupDate,settings)); });
